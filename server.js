...
app.post('/api/contacts/import', upload.single('file'), (req, res) => {
  const contacts = readJson(CONTACTS_FILE, []);
  const imported = [
    { id: `c-${Date.now()}-1`, name: '王哲遠', email: 'cheyuan.wang@company.com', department: '系統架構組', title: '軟體架構師' },
    { id: `c-${Date.now()}-2`, name: '趙雅琴', email: 'yachin.chao@company.com', department: '營運管理部', title: '營運資深總監' }
  ];
  imported.forEach(item => contacts.push(item));
  writeJson(CONTACTS_FILE, contacts);
  res.json({ success: true, addedCount: imported.length, contacts });
});
