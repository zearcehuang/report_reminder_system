...
export const EditReportModal: React.FC<Props> = ({
  ...
  const {
    ...
    getCalculatedOffset,
  } = useReportDateOffset({ activeProject, rule, isOpen });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : '更新失敗，請稍後重試';
      setErrorMsg(message);
    } finally {
      ...
      >
        <div style={{ padding: '1.25rem 1.75rem', background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: '0.2rem 0 0' }}>{activeProject.name} ({activeProject.code})</p>
            </div>
          </div>
          <button type="button" onClick={onClose} style={{ background: 'rgba(255, 255, 255, 0.1)', border: 'none', color: '#94a3b8', cursor: 'pointer', padding: '0.4rem', borderRadius: 'var(--radius-sm)', display: 'flex' }}>
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '1.5rem 1.75rem', overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            ...
            <input type="text" className="input-glass" value={title} onChange={(e) => setTitle(e.target.value)} required style={{ width: '100%', fontSize: '0.9rem', padding: '0.6rem 0.85rem', fontWeight: 600 }} />
          </div>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            ...
            <textarea rows={3} className="input-glass" value={notes} onChange={(e) => setNotes(e.target.value)} style={{ width: '100%', fontSize: '0.85rem', padding: '0.6rem 0.85rem', resize: 'vertical' }} />
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid #e2e8f0' }}>
            ...
            <button type="submit" className="btn-primary" disabled={isSubmitting} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Check size={16} /> <span>{isSubmitting ? '儲存中...' : '儲存變更'}</span>
            </button>
          </div>
        </form>
      </div>
